# -*- coding: utf-8 -*-
"""
Created on Mon Jan 21 10:32:37 2019
...
VehicleData = imp.load_source('Vehicle','../Data.py')



parser = argparse.ArgumentParser(description="show some parse for the progess")
parser.add_argument("--DataDir",type=str,default="../uci_data_set/Vehicle.mat",help="the mat data file path")
parser.add_argument("--train_epoch",type=int,default=4000,help="number of training epochs")
    ...
    for i in range(cv):
        ...
        with createVar["g"+str(i)].as_default():
            with tf.variable_scope("Graph{}".format(i)):
                createVar["xs_g"+str(i)] = tf.placeholder(tf.float32, [None, 18],name='data')
                createVar["ys_g"+str(i)] = tf.placeholder(tf.float32, [None, 4],name='target')
                createVar["weight1_g"+str(i)] = weight_variable([18,10],tf.truncated_normal,0.1,'w1')
                createVar["bais1_g"+str(i)] = bias_variable([10],'b1')
                createVar["layer1_g"+str(i)] = tf.nn.relu(tf.matmul(createVar["xs_g"+str(i)],createVar["weight1_g"+str(i)]) + createVar["bais1_g"+str(i)])
                createVar["weight2_g"+str(i)] = weight_variable([10,4],tf.truncated_normal,0.1,'w2')
                createVar["bais2_g"+str(i)] = bias_variable([4],'b2')
                createVar["output_g"+str(i)] = tf.matmul(createVar["layer1_g"+str(i)],createVar["weight2_g"+str(i)]) + createVar["bais2_g"+str(i)]
...
Vehicle_data = VehicleData.UCIData(parse.DataDir)
X,Y,partial_Y = Vehicle_data.createPartialData(rate=parse.create_partial_data_rate,nb_partial_target=parse.nb_partial_target)
X = Data.guiYiHua(X)
