# -*- coding: utf-8 -*-
"""
Created on Mon Jan 28 16:29:06 2019
...
EcoliData = imp.load_source('Ecoli','../Data.py')
    ...
    for i in range(cv):
        ...
        with createVar["g"+str(i)].as_default():
            with tf.variable_scope("Graph{}".format(i)):
                ...
                createVar["ys_g"+str(i)] = tf.placeholder(tf.float32, [None, 8],name='target')
                createVar["weight_g"+str(i)] = weight_variable([7,8],tf.truncated_normal,0.1,'w0')
                createVar["bais_g"+str(i)] = bias_variable([8],'b0')
...
Ecoli_data = EcoliData.UCIData(parse.DataDir)
X,Y,partial_Y = Ecoli_data.createPartialData(rate=parse.create_partial_data_rate,nb_partial_target=parse.nb_partial_target)
X = Data.guiYiHua(X)
...
print(Accu_array)
if parse.save2txt:
    EcoliData.UCIData.save2txt(parse,Accu_array,'Network_loss3_1_2.txt')
