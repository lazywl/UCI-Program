# -*- coding: utf-8 -*-
"""
Created on Mon Jan 28 16:56:23 2019
...
MovementData = imp.load_source('Movement','../Data.py')



parser = argparse.ArgumentParser(description="show some parse for the progess")
parser.add_argument("--DataDir",type=str,default="../uci_data_set/Movement.mat",help="the mat data file path")
parser.add_argument("--train_epoch",type=int,default=2000,help="number of training epochs")
    ...
    for i in range(cv):
        ...
        with createVar["g"+str(i)].as_default():
            with tf.variable_scope("Graph{}".format(i)):
                createVar["xs_g"+str(i)] = tf.placeholder(tf.float32, [None, 90],name='data')
                createVar["ys_g"+str(i)] = tf.placeholder(tf.float32, [None, 15],name='target')
                createVar["weight_g"+str(i)] = weight_variable([90,15],tf.truncated_normal,0.1,'w0')
                createVar["bais_g"+str(i)] = bias_variable([15],'b0')
...
Movement_data = MovementData.UCIData(parse.DataDir)
X,Y,partial_Y = Movement_data.createPartialData(rate=parse.create_partial_data_rate,nb_partial_target=parse.nb_partial_target)
X = Data.guiYiHua(X)
...
print(Accu_array)
if parse.save2txt:
    MovementData.UCIData.save2txt(parse,Accu_array,'Network_loss3_1_2.txt')
