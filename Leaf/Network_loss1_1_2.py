# -*- coding: utf-8 -*-
"""
Created on Mon Jan 28 16:35:52 2019
...
LeafData = imp.load_source('Leaf','../Data.py')



parser = argparse.ArgumentParser(description="show some parse for the progess")
parser.add_argument("--DataDir",type=str,default="../uci_data_set/Leaf.mat",help="the mat data file path")
    ...
    for i in range(cv):
        ...
        with createVar["g"+str(i)].as_default():
            with tf.variable_scope("Graph{}".format(i)):
                createVar["xs_g"+str(i)] = tf.placeholder(tf.float32, [None, 14],name='data')
                createVar["ys_g"+str(i)] = tf.placeholder(tf.float32, [None, 30],name='target')
                createVar["weight_g"+str(i)] = weight_variable([14,30],tf.truncated_normal,0.1,'w0')
                createVar["bais_g"+str(i)] = bias_variable([30],'b0')
...
Leaf_data = LeafData.UCIData(parse.DataDir)
X,Y,partial_Y = Leaf_data.createPartialData(rate=parse.create_partial_data_rate,nb_partial_target=parse.nb_partial_target)
X = Data.guiYiHua(X)
...
print(Accu_array)
if parse.save2txt:
    LeafData.UCIData.save2txt(parse,Accu_array,'Network_loss1_1_2.txt')
